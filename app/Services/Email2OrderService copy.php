...
//     public function saveEmailAttachments($email_id, $order_id)
//     {
...
//     }
    ...
    public function addEmail2Order(int $orderId, int $email_id, int $userId = 1, string $notes = 'Email automatycznie przydzielony do zgłoszenia' ) {
        Log::info('I am in addEmail2Order');
        $email2order = new EmailsToOrder(); 
        $email2order->order_id = $orderId;
        $email2order->email_id = $email_id;
        $email2order->user_id = $userId;
        $email2order->notes = $notes;
        $email2order->save();

        Email2OrderService::getEmailAttachments($email_id, $orderId);
        Log::info('I have already done the attachments');
        return $email2order;
    }

    public function changeEmailStatus($email_id) {
